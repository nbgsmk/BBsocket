...
module.exports = function createCandleRoutes(service) {
  ...
  router.get('/live', (req, res) => {
    ...
    const send = (event) => {
      ...
      const candle = event ? event.candle : candles[candles.length - 1];
      if (!includeIncomplete) {
    ...
    const unsubscribe = typeof service.subscribeAggregatedCandles === 'function'
      ? service.subscribeAggregatedCandles(requested, aggregation, { includeIncomplete, onBackfill: 'ignore', lastOpenTime: lastCompletedOpenTime }, send)
      : service.subscribeCandles(candle => { if (candle.instrument === requested) send(); });
