...
class CandleHistory {
  ...
  aggregate(symbol, aggregation) {
    ...
    return Array.from(groups.entries()).reduce((result, [openTime, group]) => {
      ...
      const complete = group.length === expected && !group.some((candle, index) => index > 0 && candle.openTime !== group[index - 1].openTime + sourceMs);
      if (!complete || openTime + targetMs > Date.now()) return result;
      result.push({
        symbol,
